...
Page({
  ...
  _getArticle: function(article_id) {
    ...
    article.getArticle(article_id, (res) => {
      wx.hideLoading();
      let oldImg = res.imgList.split(',');
      for (var i in oldImg) {
        let height = 750*oldImg[i].split('_')[2] / oldImg[i].split('_')[1];
        var mode = "aspectFit"
        if (height > 750 || height < 200) {
          mode = "aspectFill";
          height = 750
        }
        images.push({
  ...
  _previewimg: function(e) {
    ...
    for (let i in e.currentTarget.dataset.imgs){
      imgs.push(e.currentTarget.dataset.imgs[i].imgUrl)
    }
    console.log(imgs)
    wx.previewImage({
  ...
  onShareAppMessage: function(e) {
    ...
    var param = {
      type: that.data.articleType ? 'video' : 'article',
